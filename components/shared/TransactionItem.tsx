  ...
  return (
    <li
      className={`flex justify-between pb-space-250 border-b-1 last:border-none ${
        variant === "budgets" ? "border-alt/15" : "border-default"
      }`}
    >
      <div className="flex items-center gap-space-200">
        <div
          className={`relative w-8 h-8 md:w-10 md:h-10 ${
            variant === "budgets" && "hidden md:block"
          }`}
        >
          <Image
            src={avatar.replace("./", "/")}
      ...
      <div className="grid gap-space-100">
        <span
          className={`text-preset-4 tracking-preset-4 leading-preset-4 font-preset-4-bold text-right ${
